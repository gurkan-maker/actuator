...
# Add ball valve constants
BALL_VALVE_POSITIONS = {
    "BTO": 1.0,    # Break open torque
    "RTO": 0.7,    # Running open torque
    "ETO": 0.7,    # End to open torque
    "ETC": 0.85,   # End to close torque
    "RTC": 0.5,    # Running close torque
    "MAST": 1.0    # Maximum stem torque
}

SEAT_MATERIAL_MULTIPLIERS = {
...
FLUID_TYPE_MULTIPLIERS = {
    "Gas or superheated steam, clean / dry": 1.25,
    "Viscous fluid": 1.5,
    "Liquids containing abrasive solids": 1.65,
...
FREQUENCY_MULTIPLIERS = {
    ...
    "Critical ESD services": 1.8
}

# Update valve factors to match Guide.pdf
VALVE_FACTORS = {
    "Parallel Slide Gate": {
        "Liquid": {"Below 400°C": 0.25, "Above 400°C": 0.30},
        "Gas/Steam": {"Below 400°C": 0.35, "Above 400°C": 0.45}
    },
    "Flex Wedge Gate": {
        "Liquid": {"Below 400°C": 0.30, "Above 400°C": 0.40},
        "Gas/Steam": {"Below 400°C": 0.45, "Above 400°C": 0.50}
    },
    "Solid Wedge Gate": {
        "Liquid": {"Below 400°C": 0.35, "Above 400°C": 0.40},
        "Gas/Steam": {"Below 400°C": 0.45, "Above 400°C": 0.50}
    },
    "Globe": {
    ...
    "Ball": {
        "Liquid": {"Below 400°C": 0.35, "Above 400°C": 0.35},
        "Gas/Steam": {"Below 400°C": 0.25, "Above 400°C": 0.25}
    }
}

# Add stem factors from Table 2 in Guide.pdf
STEM_FACTORS = {
    "3/4": 0.007,
    "7/8": 0.007,
    "1": 0.008,
...
class Valve:
    def __init__(self, size_inch, valve_type, pressure_class, seat_material, stem_dia_mm, 
                 ...
                 ball_type=None, trim_no=None):  # NEW: ball_type and trim_no
        self.size = size_inch
        ...
        self.ball_type = ball_type  # "Floating" or "Trunnion"
        self.trim_no = trim_no      # For Parallel Slide Gate valves
...
VALVE_DATABASE = [
    Valve(2, "Ball", 600, "PTFE", 20, 100, 0, 200, -20, "SS316", None, None, "Floating", None),
    Valve(4, "Ball", 600, "Metal", 30, 100, 0, 250, -20, "SS316", None, None, "Trunnion", None),
    Valve(6, "Butterfly", 300, "Elastomer", 40, 40, 0, 150, -10, "Carbon Steel", None, None, None, None),
    Valve(8, "Butterfly", 150, "PTFE", 50, 25, 0, 180, -10, "SS304", None, None, None, None),
    Valve(3, "Globe", 900, "Graphite", 25, 150, 0, 350, -50, "Alloy Steel", 40, 200, None, None),
    Valve(6, "Gate", 600, "Graphite", 35, 100, 0, 400, -30, "Alloy Steel", 45, 300, None, 5),
    Valve(2, "Plug", 600, "PTFE", 22, 100, 0, 200, -20, "SS316", None, None, None, None),
    Valve(10, "Diaphragm", 150, "Elastomer", 60, 16, 0, 120, -10, "SS304", 65, 150, None, None)
]
...
def calculate_ball_valve_torque(valve, pressure_bar, temperature_c, 
                                position="BTO", fluid_type="Clean / non-lubricating",
                                frequency="Once per day or greater"):
    """Calculate torque for ball valves with position-specific factors"""
    # Base torque calculation (Nm)
    base_torque = valve.size * pressure_bar * 0.8
    
    # Get position factor
    position_factor = BALL_VALVE_POSITIONS.get(position, 1.0)
    
    # Get multipliers
    seat_multiplier = SEAT_MATERIAL_MULTIPLIERS.get(valve.seat_material, 1.0)
    fluid_multiplier = FLUID_TYPE_MULTIPLIERS.get(fluid_type, 1.0)
    frequency_multiplier = FREQUENCY_MULTIPLIERS.get(frequency, 1.0)
    
    # Temperature multiplier
    if temperature_c >= 100 and temperature_c <= 218:
        temp_multiplier = temperature_c / 100
    elif temperature_c < 0 or temperature_c > 218:
        temp_multiplier = 1.5  # Conservative estimate for extreme temps
    else:
        temp_multiplier = 1.0
    
    # Apply all factors
    torque = (base_torque * position_factor * seat_multiplier * 
              fluid_multiplier * frequency_multiplier * temp_multiplier)
    
    # Adjust for trunnion vs floating
    if valve.ball_type == "Trunnion":
        torque *= 0.7  # Trunnion typically requires less torque
...
def get_valve_factor(valve_type, media_type, temperature_c, gate_type=None, trim_no=None):
    """Get valve factor C with trim number support"""
    temp_category = "Above 400°C" if temperature_c > 400 else "Below 400°C"
    
    # Handle Parallel Slide Gate with trim numbers
    if valve_type == "Gate" and "Parallel" in gate_type and trim_no is not None:
        if trim_no == 5:
            return 0.20
        elif trim_no == 7:
            return 0.15
    
    # Standard valve factor lookup
    if valve_type == "Gate":
        # Map to the appropriate key in VALVE_FACTORS
        if gate_type == "Solid Wedge":
            key = "Solid Wedge Gate"
        elif gate_type == "Flexible Wedge":
            key = "Flex Wedge Gate"
        else:
            key = "Parallel Slide Gate"
            
        if key in VALVE_FACTORS:
            return VALVE_FACTORS[key][media_type][temp_category]
    
    # For other valve types
    if valve_type in VALVE_FACTORS:
        return VALVE_FACTORS[valve_type][media_type][temp_category]
...
def calculate_gate_valve_thrust(valve, pressure_bar, temperature_c, media_type, gate_type=None):
    ...
    # Get valve factor C
    C = get_valve_factor("Gate", media_type, temperature_c, gate_type, valve.trim_no)
    ...
    # Piston effect (F) - only for gate valves and pressure >= 1000 PSI
    piston_effect_lbf = 0
...
def get_stem_factor(stem_dia_inch, lead):
    """Get stem factor using interpolation from Table 2"""
    # Find closest standard sizes
    sizes = sorted(STEM_FACTORS.keys(), key=lambda x: float(x.split()[0]))
    closest_size = min(sizes, key=lambda x: abs(float(x.split()[0]) - stem_dia_inch))
    
    # Get base factor
    base_factor = STEM_FACTORS[closest_size]
    
    # Adjust for lead (simplified)
    if lead > 0.25:  # Coarse thread
        return base_factor * 0.8
    return base_factor  # Fine thread

def calculate_valve_torque_thrust(valve, pressure_bar, temperature_c, media_type, gate_type=None,
                                  ball_position="BTO", fluid_type="Clean / non-lubricating",
                                  frequency="Once per day or greater"):
    """Calculate torque or thrust based on valve type"""
    if valve.type == "Ball" or valve.type == "Plug":
        torque = calculate_ball_valve_torque(
            valve, pressure_bar, temperature_c, 
            ball_position, fluid_type, frequency
        )
...
# ========================
# QUARTER-TURN GEARBOX CALCULATIONS
# ========================
def calculate_gearbox_torque(input_torque, gear_ratio, efficiency=0.85):
    """Calculate gearbox output torque"""
    return input_torque * gear_ratio * efficiency

def calculate_handwheel_diameter(output_torque, rim_effort, gear_ratio):
    """Calculate required handwheel diameter"""
    return (output_torque * 2) / (rim_effort * gear_ratio)
...
def plot_torque_thrust_vs_pressure(valve, temperature_c, max_pressure, media_type, gate_type=None,
    ...
    for pressure in pressures:
        value, value_type = calculate_valve_torque_thrust(
            valve, pressure, temperature_c, media_type, gate_type,
            ball_position, fluid_type, frequency
        )
        values.append(value)
...
def main():
    ...
    with st.sidebar:
        ...
        if valve_selection_method == "Select from Database":
            ...
            # Ball valve specific
            if valve_type == "Ball":
                ball_type = st.selectbox("Ball Type", ["Floating", "Trunnion"])
            
            # Gate valve specific
            if valve_type == "Gate":
                gate_subtype = st.selectbox("Gate Subtype", ["Solid Wedge", "Flexible Wedge", "Parallel Slide"])
                if "Parallel" in gate_subtype:
                    trim_no = st.selectbox("Trim Number", [None, 5, 7], format_func=lambda x: str(x) if x else "Standard")
            
            # Linear valve parameters
            ...
            selected_valve = Valve(
                ...
                stem_material, pitch_dia, stroke, ball_type, trim_no
            )
        ...
        # Ball valve specific parameters
        ball_position = None
        fluid_type = None
        frequency = None
        if selected_valve.type == "Ball":
            ball_position = st.selectbox("Torque Position", list(BALL_VALVE_POSITIONS.keys()))
            fluid_type = st.selectbox("Fluid Type", list(FLUID_TYPE_MULTIPLIERS.keys()))
            frequency = st.selectbox("Frequency of Operation", list(FREQUENCY_MULTIPLIERS.keys()))
        
        # Gate type selection only for gate valves
        gate_type = None
        if selected_valve.type == "Gate":
            gate_type = st.selectbox("Gate Type", ["Solid Wedge", "Flexible Wedge", "Parallel Slide"])
        ...
        if selected_valve.ball_type:
            st.markdown(f"**Ball Type:** {selected_valve.ball_type}")
        if selected_valve.trim_no:
            st.markdown(f"**Trim Number:** {selected_valve.trim_no}")
    ...
    # Main content area
    tab1, tab2, tab3, tab4 = st.tabs(["Calculation", "Actuator Selection", "Stem Analysis", "Gearbox"])
    ...
    with tab1:
        ...
        if st.session_state.results:
            ...
            fig = plot_torque_thrust_vs_pressure(
                ...
                results["ball_position"] if "ball_position" in results else "BTO",
                results["fluid_type"] if "fluid_type" in results else "Clean / non-lubricating",
                results["frequency"] if "frequency" in results else "Once per day or greater"
            )
            ...
            with st.expander("View calculation parameters"):
                ...
                if results['valve'].type == "Ball":
                    st.markdown(f"**Ball Position:** {results['ball_position']}")
                    st.markdown(f"**Fluid Type:** {results['fluid_type']}")
                    st.markdown(f"**Frequency:** {results['frequency']}")
                    st.markdown(f"**Ball Type:** {results['valve'].ball_type}")
                
                if results['valve'].type == "Gate":
                    st.markdown(f"**Gate Type:** {results['gate_type']}")
                    if results['valve'].trim_no:
                        st.markdown(f"**Trim Number:** {results['valve'].trim_no}")
    ...
    with tab4:
        st.subheader("Quarter-Turn Gearbox Calculations")
        st.markdown("Calculate output torque and handwheel diameter for gearboxes")
        
        col1, col2 = st.columns(2)
        with col1:
            input_torque = st.number_input("Input Torque (Nm)", min_value=0.0, value=100.0)
            gear_ratio = st.number_input("Gear Ratio", min_value=1.0, value=10.0)
            efficiency = st.number_input("Efficiency (%)", min_value=0.1, max_value=1.0, value=0.85)
            
            output_torque = calculate_gearbox_torque(input_torque, gear_ratio, efficiency)
            st.metric("Output Torque", f"{output_torque:.2f} Nm")
        
        with col2:
            rim_effort = st.number_input("Desired Rim Effort (N)", min_value=10.0, value=150.0)
            hw_diameter = calculate_handwheel_diameter(output_torque, rim_effort, gear_ratio)
            st.metric("Required Handwheel Diameter", f"{hw_diameter:.2f} m")
    ...
    if export_btn and st.session_state.results:
        try:
            ...
            if valve.ball_type:
                valve_details.append(("Ball Type:", valve.ball_type))
            if valve.trim_no:
                valve_details.append(("Trim Number:", str(valve.trim_no)))
            ...
            if st.session_state.results.get('ball_position'):
                op_conditions.append(("Ball Position:", st.session_state.results['ball_position']))
                op_conditions.append(("Fluid Type:", st.session_state.results['fluid_type']))
                op_conditions.append(("Frequency:", st.session_state.results['frequency']))
